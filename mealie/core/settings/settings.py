...
class AppSettings(AppLoggingSettings):
    ...
    # LLM gateway
    #
    # Provider routing, credentials, and model selection live in the
    # LiteLLM sidecar (see ADR-0002). The application only knows the
    # sidecar's URL and master key; it does not know which provider
    # serves a given request.

    LLM_BASE_URL: str | None = None
    """URL of the LLM gateway (e.g. http://litellm:4000)"""
    LLM_API_KEY: MaskedNoneString = None
    """API key for the LLM gateway"""

    OPENAI_ENABLE_IMAGE_SERVICES: bool = True
    """Whether to enable image-related LLM features"""
    OPENAI_ENABLE_TRANSCRIPTION_SERVICES: bool = True
    """Whether to enable audio-transcription LLM features"""
    OPENAI_WORKERS: int = 2
    """
    Number of LLM workers per request. Higher values may increase
    processing speed, but will incur additional sidecar load
    """
    ...
    def OPENAI_FEATURE(self) -> FeatureDetails:
        if not self.LLM_BASE_URL:
            return FeatureDetails(enabled=False, description="LLM_BASE_URL is not set")
        if not self.LLM_API_KEY:
            return FeatureDetails(enabled=False, description="LLM_API_KEY is not set")
